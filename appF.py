...
# ─────────────────────────────────────────────
# CONNEXION KAFKA — CORRECTION PRINCIPALE
# ─────────────────────────────────────────────
@st.cache_resource
def load_kafka_consumer():
    """
    FIX : Le consumer écoute 'parking-results' (réponses du worker),
    PAS 'parking-alerts' (topic d'envoi).
    auto_offset_reset='latest' pour ne lire que les nouveaux messages.
    """
    try:
        consumer = KafkaConsumer(
            'parking-results',           # ← FIX : topic des résultats worker
            bootstrap_servers=['localhost:9092'],
            auto_offset_reset='latest',
            enable_auto_commit=True,
            group_id='streamlit-video-consumer',
...
@st.cache_resource
def load_kafka_consumer_live():
    """
    Consumer dédié à la page Alertes Live — écoute 'parking-alerts'.
    Séparé du consumer vidéo pour éviter les conflits de group_id.
    """
    try:
        consumer = KafkaConsumer(
            'parking-alerts',
            ...
            auto_offset_reset='latest',
            enable_auto_commit=True,
            group_id='streamlit-live-alerts',
            value_deserializer=lambda x: json.loads(x.decode('utf-8')),
            consumer_timeout_ms=1500
        )
        return consumer, "✅ Kafka Live connecté"
    except Exception as e:
        return None, f"❌ Kafka Live erreur : {e}"

@st.cache_resource
def load_kafka_producer():
    """
    FIX : value_serializer gère à la fois bytes (frames JPEG) et dict (JSON).
    acks='all' + retries=3 pour fiabilité.
    """
    try:
...
kafka_consumer_live, _                  = load_kafka_consumer_live()
...
# ─────────────────────────────────────────────
# HELPER : envoyer alerte Kafka
# ─────────────────────────────────────────────
def kafka_send_alert(label, level, score, source):
    """Envoie une alerte sur 'parking-alerts' et met à jour les métriques."""
...
# ─────────────────────────────────────────────
# HELPER : mise à jour métriques détection
# ─────────────────────────────────────────────
def update_metrics(label, score):
    init_metrics()
    m = st.session_state.metrics
    m["total_detections"] += 1
    if label == "fire":
        m["fire_detections"] += 1
        m["scores_fire"].append(round(score, 3))
    elif label == "smoke":
        m["smoke_detections"] += 1
        m["scores_smoke"].append(round(score, 3))
    else:
        m["false_negatives"] += 1
    m["timeline"].append({
        "label": label,
...
# ─────────────────────────────────────────────
# HELPER : déclencher alarme + sauvegarder
# ─────────────────────────────────────────────
def handle_alert(label, level, score, source):
    """Déclenche l'alarme, sauvegarde MySQL, envoie Kafka — UNE SEULE FOIS."""
    trigger_alarm(level)

    # Anti-doublon : n'ajoute que si différent du dernier
    ...
    if not hist or hist[-1]["objet"] != label or hist[-1]["level"] != level:
        hist.append(new_alert)
    ...
    kafka_send_alert(label, level, score, source)
...
MEMBRES = [
    {"emoji": "👩‍💻", "nom": "Asmae AALIKANE",
     ...
     "desc": "Modèle IA : Entraînement d'un modèle YOLOv8 pour la détection du feu. API : Transformation du flux vidéo en données exploitables. Docker : Conteneurisation de Kafka et Zookeeper. Kafka : Transmission temps réel des alertes. Sécurité : Persistance des messages pour éviter toute perte. Consommateur : Exploitation des alertes pour un dashboard."},
    {"emoji": "👩‍🔬", "nom": "Assietou ABDERRAHMANE",
     "role": "Data Scientist · Système Alert",
    ...
    {"emoji": "👩‍🎨", "nom": "Rim BARI",
     ...
     "desc": "Développement de l'interface Streamlit, intégration complète YOLO + Kafka en temps réel et conception du tableau de bord d'analyse + Connection avec base de données."},
]
...
with st.sidebar:
    ...
    global_conf = st.slider(
        ...
        0.05,
        format="%.2f",
        help="Appliqué à toutes les détections YOLO",
        label_visibility="collapsed"
    )
...
if page == "🏠  Accueil & Équipe":
    ...
    with col_r:
        ...
        for ico, titre, detail in [
            ("🎯", "Précision ≥ 85%",     "mAP@0.5 sur les 2 classes feu"),
    ...
    m2.metric("🏷️ Classes Feu",     "2",        "Flamme · Fumée")
...
# ═══════════════════════════════════════════════
    ...
    with tab1:
        ...
        <div style='background:var(--bg-card);border:1px solid var(--border);border-radius:10px;padding:18px;margin-bottom:14px;'>
            ...
            Mode YOLO : détection directe avec best.pt &nbsp;|&nbsp; Mode Kafka : frames envoyées au broker Docker → résultats lus depuis <b>parking-results</b>.
            </span>
        </div>
        """, unsafe_allow_html=True)
        ...
        if video_file:
            if st.session_state.detection_mode == "YOLO" and not yolo_ok:
                st.error("❌ Mode YOLO sélectionné mais best.pt introuvable.")
                st.stop()
            if st.session_state.detection_mode == "KAFKA" and not kafka_ok:
                st.warning("⚠️ Kafka non disponible. Basculement en mode YOLO.")
                st.session_state.detection_mode = "YOLO"

            st.success(f"✅ Vidéo chargée — Mode : **{st.session_state.detection_mode}**")
            ...
            init_metrics()
            # Reset métriques pour la nouvelle session vidéo
            st.session_state.metrics = {
                "total_detections": 0, "fire_detections": 0, "smoke_detections": 0,
                "false_negatives":  0, "scores_fire": [], "scores_smoke": [],
                "kafka_sent": 0, "kafka_received": 0, "latencies_ms": [], "timeline": [],
            ...
            while cap.isOpened() and st.session_state.get("video_running", False):
                ...
                if st.session_state.detection_mode == "YOLO" and yolo_ok:
                    ...
                    level, _ = classify_risk(label)
                    display_frame = annotated if show_boxes else frame

                # ── MODE KAFKA ──
                elif st.session_state.detection_mode == "KAFKA":
                    if kafka_ok and kafka_producer:
                        try:
                            # FIX : encoder la frame en JPEG et l'envoyer sur 'parking-frames'
                            _, buf = cv2.imencode('.jpg', frame,
                                                  [cv2.IMWRITE_JPEG_QUALITY, 70])
                            future = kafka_producer.send(
                                'parking-frames',
                                value=buf.tobytes(),
                                key=str(frame_idx).encode()
                            )
                            future.get(timeout=2)   # attendre ACK
                            st.session_state.metrics["kafka_sent"] += 1

                            # FIX : lire la réponse depuis 'parking-results'
                            records = kafka_consumer.poll(timeout_ms=500)
                            for tp, messages in records.items():
                                for msg in messages:
                                    data = msg.value
                                    if isinstance(data, dict):
                                        label = data.get('label', data.get('objet', 'normal'))
                                        score = float(data.get('score', data.get('confiance', 0.0)))
                                        level, _ = classify_risk(label)
                                        st.session_state.metrics["kafka_received"] += 1

                        except KafkaTimeoutError:
                            pass   # frame ignorée silencieusement
                        except Exception as e:
                ...
                # ── Métriques ──
                update_metrics(label, score)
                if label == "fire":
                    flame_count += 1
                elif label == "smoke":
                    smoke_count += 1

                # ── Alarme + sauvegarde (une seule fois) ──
                if level in ("CRITIQUE", "ALERTE"):
                    handle_alert(label, level, score, "VideoUpload")
                ...
                if show_scores and score > 0:
                    cv2.putText(
                        ...
                        (0, 80, 255) if level == "CRITIQUE" else (0, 200, 80), 2
                    )

                frame_ph.image(frame_display, channels="BGR", use_container_width=True)
            ...
            # Sauvegarde session MySQL
            if mysql_ok:
                m = st.session_state.metrics
                avg_fire  = sum(m["scores_fire"])  / len(m["scores_fire"])  if m["scores_fire"]  else 0
                avg_smoke = sum(m["scores_smoke"]) / len(m["scores_smoke"]) if m["scores_smoke"] else 0
                save_session(
                    session_id = st.session_state["session_id"],
                    fire       = flame_count,
                    smoke      = smoke_count,
...
"""             # • <b style='color:#ffbf00;'>Android USB</b> : Installe <b>IP Webcam</b> sur ton téléphone → Lance le serveur
 ...
 """            • <b style='color:#ffbf00;'>Android Wi-Fi</b> : Même réseau Wi-Fi que le PC → entre l'IP affichée dans IP Webcam<br>
            • <b style='color:#ffbf00;'>Webcam PC</b> : Sélectionne "Webcam PC intégrée" "
            </span>
        </div>
        """, unsafe_allow_html=True)
        ...
        with col_src:
            ...
            #     </div>
            #     """, unsafe_allow_html=True)
            if source_type == "📡 Android Wi-Fi (IP Webcam)":
                ...
                rtsp_url = st.text_input("🌐 URL RTSP", value="rtsp://",
                                         placeholder="rtsp://user:pass@192.168.1.x:554/stream")

        with col_cam2:
            cam_conf = st.slider("Seuil confiance", 0.10, 1.00,
                                 st.session_state.get("global_conf", 0.25), 0.05)
        ...
        col_start, col_stop = st.columns(2)
        with col_start:
            if st.button("▶  Démarrer la caméra", type="primary"):
        ...
        if st.session_state.get("cam_running", False):
            if not yolo_ok:
                ...
                cap_cam.set(cv2.CAP_PROP_BUFFERSIZE, 1)
                ...
                if not cap_cam.isOpened():
                    ...
                    while st.session_state.get("cam_running", False):
                        ...
                        if frame_count % skip_frames == 0:
                            label_c, score_c, annotated_c, _ = get_yolo_prediction(
                                yolo_model, frame_c, cam_conf
                            )
                            level_c, _ = classify_risk(label_c)

                            if level_c in ("CRITIQUE", "ALERTE"):
                                handle_alert(label_c, level_c, score_c, source_label)

                            update_metrics(label_c, score_c)
                            if label_c == "fire":   cam_flame += 1
                            elif label_c == "smoke": cam_smoke += 1

                            color_cv = (0, 0, 255)   if level_c == "CRITIQUE" else \
                                       (0, 165, 255) if level_c == "ALERTE"   else (0, 200, 80)
                            cv2.putText(annotated_c,
                                        f"{label_c.upper()} | {score_c:.2f} | {level_c}",
                                        (15, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.9, color_cv, 2)
                            cv2.putText(annotated_c, time.strftime("%H:%M:%S"),
                    ...
                    st.info(f"✅ Caméra arrêtée — Fire={cam_flame} | Smoke={cam_smoke} | Frames={frame_count}")
        ...
        if img_file:
            ...
            if st.session_state.get("image_analyzed", False):
                if yolo_ok:
                    ...
                    level_i, _ = classify_risk(label_i)
                    ...
                    <div class='{alert_class}'>
                        ...
                        <span style='font-size:.8rem;color:#8a7260;'>{len(dets_i)} détection(s) au total</span>
                    </div>
                    """, unsafe_allow_html=True)

                    if level_i in ("CRITIQUE", "ALERTE"):
                        handle_alert(label_i, level_i, score_i, "ImageUpload")
                        if kafka_ok and kafka_producer:
    ...
    with tab_live:
        if not kafka_ok:
            ...
            if start_live:
                ...
                msg_count = 0
                # FIX : utiliser le consumer dédié au live (group_id séparé)
                for message in kafka_consumer_live:
...
# ═══════════════════════════════════════════════
    ...
    combined_scores = m["scores_fire"] + m["scores_smoke"]
    avg_conf_global = round(sum(combined_scores) / len(combined_scores), 3) if combined_scores else 0.0
    ...
    with col_yolo:
        ...
        <div style='background:var(--bg-card);border:1px solid var(--border);
            ...
            <div class='pb-wrap'><div class='pb-fill' style='width:{avg_conf_fire*100:.1f}%'></div></div>
        ...
        <div style='background:var(--bg-card);border:1px solid var(--border);
            ...
            <div class='pb-wrap'><div class='pb-fill' style='width:{avg_conf_smoke*100:.1f}%'></div></div>
    ...
    with col_kafka:
        ...
        taux = f"{(m['kafka_received']/m['kafka_sent']*100):.0f}%" if m["kafka_sent"] > 0 else "—"
        ...
        <div style='background:var(--bg-card);border:1px solid var(--border);
            ...
            <div style='display:grid;grid-template-columns:1fr 1fr;gap:12px;'>
                ...
                <div style='text-align:center;background:#1a0f08;border-radius:8px;padding:12px;'>
                    <div style='font-family:"Bebas Neue",sans-serif;font-size:1.6rem;color:#ff8c00;'>{taux}</div>
        ...
        <div style='background:var(--bg-card);border:1px solid var(--border);
            ...
            <div style='font-size:.82rem;line-height:2;color:#c4a882;'>
                <span style='color:#5a3a28;'>Frames topic :</span> <b>parking-frames</b><br>
                <span style='color:#5a3a28;'>Résultats topic :</span> <b>parking-results</b><br>
                <span style='color:#5a3a28;'>Alertes topic :</span> <b>parking-alerts</b><br>
                <span style='color:#5a3a28;'>Broker :</span> localhost:9092<br>
                <span style='color:#5a3a28;'>Format frames :</span> JPEG bytes<br>
                <span style='color:#5a3a28;'>Format alertes :</span> JSON (objet, score, heure, source)
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with tab_conf:
        if m["scores_fire"] or m["scores_smoke"]:
            ...
            st.info("📤 Lancez une détection vidéo ou image pour voir les scores.")
    ...
    with tab_dist:
        if total > 0:
            ...
            data = [(s, l, c, e) for s, l, c, e in
                    zip([fire, smoke, normal],
                        ['🔥 Fire', '💨 Smoke', '✅ Normal'],
                        ['#ff4500', '#8a8a8a', '#22c55e'],
                        [0.05, 0.05, 0]) if s > 0]
            if data:
                sizes, lbs, cols, explode = zip(*data)
                wedges, texts, autotexts = ax.pie(
                    sizes, labels=lbs, colors=cols, explode=explode,
    ...
    with tab_f1:
        if total >= 2:
            ...
            for i, event in enumerate(m["timeline"], 1):
                if event["label"] == "fire":   rf += 1
                elif event["label"] == "smoke": rs += 1
                else: rfn += 1
                p = (rf + rs) / i
                d = rf + rs + rfn
                r = (rf + rs) / d if d > 0 else 0
    ...
    total_ref        = m["total_detections"]
    fire_ref         = m["fire_detections"]
    smoke_ref        = m["smoke_detections"]
    fn_ref           = m["false_negatives"]
    scores_fire_ref  = m["scores_fire"]
    scores_smoke_ref = m["scores_smoke"]
    ...
    r1.metric("mAP@0.5",        map_val,    f"{map_dyn - 0.865:+.1%} vs entraînement"           if using_real else "+6.5% vs baseline")
    r2.metric("Précision moy.", prec_val,   f"{prec_global_dyn - 0.85:+.1%} vs entraînement"   if using_real else "")
    r3.metric("Rappel moyen",   recall_val, f"{recall_global_dyn - 0.835:+.1%} vs entraînement" if using_real else "")
    r4.metric("F1-Score",       f1_val,     f"{f1_global_dyn - 0.842:+.1%} vs entraînement"     if using_real else "")

    classes_ref  = ['Fire', 'Smoke']
    couleurs_ref = ['#ff4500', '#8a8a8a']
    precision_ref = [prec_fire_dyn,   prec_smoke_dyn]  if using_real else [0.90, 0.82]
    rappel_ref    = [recall_fire_dyn, recall_smoke_dyn] if using_real else [0.88, 0.80]
    f1_ref        = [f1_fire_dyn,     f1_smoke_dyn]     if using_real else [0.89, 0.81]
        ...
        with c1:
            ...
            <div style='background:var(--bg-card);border-left:3px solid #ff4500;
                ...
                <div style='font-size:.75rem;color:#5a3a28;margin-top:6px;'>
                    {fire_ref} détections · seuil {st.session_state.get("global_conf", 0.25):.0%}
                </div>
            </div>
            """, unsafe_allow_html=True)
        with c2:
            ...
            <div style='background:var(--bg-card);border-left:3px solid #8a8a8a;
                ...
                <div style='font-size:.75rem;color:#5a3a28;margin-top:6px;'>
                    {smoke_ref} détections · seuil {st.session_state.get("global_conf", 0.25):.0%}
                </div>
            </div>
            """, unsafe_allow_html=True)
